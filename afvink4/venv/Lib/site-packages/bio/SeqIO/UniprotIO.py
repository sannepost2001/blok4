    ...
    def parse(self):
        ...
        def _parse_protein(element):
            ...
            for protein_element in element:
                if protein_element.tag in [NS + 'recommendedName', NS + 'submittedName',
                                           NS + 'alternativeName']:  # recommendedName tag are parsed before
            ...
            for organism_element in element:
                if organism_element.tag == NS + 'name':
                    ...
                    self.ParsedSeqRecord.dbxrefs.append(
                        organism_element.attrib['type'] + ':' + organism_element.attrib['id'])
                    ...
                    for el in subloc_element:
                        if el.text:
                            ann_key = 'comment_%s_%s' % (
                                element.attrib['type'].replace(' ', ''), el.tag.replace(NS, ''))
